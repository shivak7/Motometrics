...
function [Master_data] = nonlegacy_b_emg_analyze(Master_data,proc_select) 

for i = 1:numel(Master_data.expt)
    
    Vname = load([Master_data.expt(i).file '.mat']);  % This code section just extracts the data, sampling rate, frame info
...
%    orig_frames            = fz.frames;
    ...
    data_start_range = 1 + Master_data.expt(i).start_msec * (Master_data.expt(i).Fs/1000); % Create indices to extract emg data within the specified time intervals
    data_end_range = 1 + Master_data.expt(i).end_msec * (Master_data.expt(i).Fs/1000);
